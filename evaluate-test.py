...
class SQAlxtDataset(Dataset):
    def __init__(self, data_dir):
        ...
        for context_id, start_idx, end_idx in tqdm(zip(context_id, code_start, code_end)):
            ...
            context = np.loadtxt(
                os.path.join(code_passage_dir, '-'.join(context_id.split('-') + ['c']) + '.code')).astype(int)
            question_id = '-'.join(context_id.split('-') + ['q'])
            question = np.loadtxt(os.path.join(code_dir, question_id + '.code')).astype(int)
            context_cnt = np.loadtxt(
                os.path.join(code_passage_dir, '-'.join(context_id.split('-') + ['c']) + '.cnt')).astype(int)
...
def collate_dev_fn(batch):
    ...
    context_cnt = pad_sequence([torch.tensor(example['context_cnt']) for example in batch], batch_first=True,
                               padding_value=0)
...
def idx2sec(pred_start_idx, pred_end_idx, context_begin, context_cnt):
    ...
    start_frame_idx = torch.repeat_interleave(torch.ones(context_cnt[:pred_start_idx - context_begin].size()),
                                              context_cnt[:pred_start_idx - context_begin])
    end_frame_idx = torch.repeat_interleave(torch.ones(context_cnt[:pred_end_idx - context_begin].size()),
                                            context_cnt[:pred_end_idx - context_begin])
...
output_df = pd.DataFrame(
    list(zip(df['hash'].values, start_secs, end_secs, pred_starts, pred_ends, f1s_after_sec, AOSs, dup)),
